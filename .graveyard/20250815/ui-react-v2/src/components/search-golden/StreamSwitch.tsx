...
interface Props {
  ...
  onStatusUpdate: (connected: boolean, lastEventTs?: number) => void;
}
    ...
    <div style={{ padding: "10px", borderBottom: "1px solid #ccc" }}>
      <div style={{ display: "flex", alignItems: "center", marginBottom: "10px" }}>
        <label style={{ display: "flex", alignItems: "center" }}>
          ...
          />
          <span style={{ marginLeft: "5px" }}>Live Tail</span>
        </label>
        
        <div data-testid="tail-status" style={{ marginLeft: "20px" }}>
          Status: {connected ? "Connected" : enabled ? "Connecting..." : "Disconnected"}
          {lastEventTs && (
            <span style={{ marginLeft: "10px", color: "#666" }}>
              Last event: {new Date(lastEventTs).toLocaleTimeString()}
            </span>
          )}
        </div>
      </div>
