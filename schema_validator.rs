        ...
        for meta_col in meta {
            if let Some(col_name) = meta_col.get("name").and_then(|n| n.as_str()) {
                if col_name == "assignee_id" {
                    return Err(
                        "❌ Found deprecated 'assignee_id' column in alerts response".into(),
                    );
                }
            }
        }
