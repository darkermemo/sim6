...
api /api/v2/admin/tenants "$(jq -n --arg id "$TENANT" --arg name "$TENANT" '{tenant_id:$id, name:$name}')" | tee "$ART_DIR/tenants_create.json" >/dev/null

note "set limits"
api_put "/api/v2/admin/tenants/$TENANT/limits" \
