...
mod tests {
    ...
    fn test_network_event_deserialization() {
        ...
        let event: ParsedEvent = serde_json::from_value(json_data)
            .expect("Failed to deserialize network event");
        ...
        assert_eq!(event.event_action, Some("connection_established".to_string()));
    ...
    fn test_web_event_deserialization() {
        ...
        let event: ParsedEvent = serde_json::from_value(json_data)
            .expect("Failed to deserialize web event");

        assert_eq!(event.source_ip, Some("203.0.113.45".to_string()));
        assert_eq!(event.http_response_status_code, Some(404));
        assert_eq!(event.url_original, Some("https://example.com/admin/login".to_string()));
        assert_eq!(event.http_method, Some("POST".to_string()));
        assert_eq!(event.user_agent, Some("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36".to_string()));
    ...
    fn test_process_event_deserialization() {
        ...
        let event: ParsedEvent = serde_json::from_value(json_data)
            .expect("Failed to deserialize process event");
        ...
        assert_eq!(event.file_path, Some("C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe".to_string()));
        assert_eq!(event.file_name, Some("powershell.exe".to_string()));
        assert_eq!(event.file_hash_sha256, Some("a1b2c3d4e5f6789012345678901234567890abcdef1234567890abcdef123456".to_string()));
    ...
    fn test_security_event_with_additional_fields() {
        ...
        let event: ParsedEvent = serde_json::from_value(json_data)
            .expect("Failed to deserialize security event");
        ...
        assert_eq!(event.rule_name, Some("Suspicious Network Activity".to_string()));
        assert_eq!(event.threat_name, Some("Malware.Generic.Trojan".to_string()));
        ...
        assert_eq!(event.get_field("analyst_notes"), Some(&json!("Confirmed malicious activity from known botnet")));
    }
    ...
    fn test_complete_event_lifecycle_and_validation() {
        ...
        let deserialized_event = ParsedEvent::from_json(&json_string)
        ...
        assert_eq!(event.additional_fields, deserialized_event.additional_fields);
        
        // Validate additional fields preservation
        assert_eq!(deserialized_event.get_field("query_type"), Some(&json!("A")));
        assert_eq!(deserialized_event.get_field("response_code"), Some(&json!(0)));
        assert_eq!(deserialized_event.get_field("query_domain"), Some(&json!("malicious-domain.com")));
        assert_eq!(deserialized_event.get_field("blocked"), Some(&json!(true)));
        
        // Test pretty JSON formatting
        let pretty_json = event.to_json_pretty().expect("Failed to serialize pretty JSON");
