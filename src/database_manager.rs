...
use crate::error_handling::{DatabaseHealthChecker, RetryPolicy, SiemError, SiemResult};
...
impl DatabaseConfig {
    ...
    pub fn from_env() -> SiemResult<Self> {
        ...
        let database = std::env::var("CLICKHOUSE_DATABASE").map_err(|_| {
            SiemError::configuration("CLICKHOUSE_DATABASE environment variable not set")
        })?;

        let username = std::env::var("CLICKHOUSE_USERNAME").map_err(|_| {
            SiemError::configuration("CLICKHOUSE_USERNAME environment variable not set")
        })?;
        ...
        if database.trim().is_empty() {
            return Err(SiemError::configuration(
                "CLICKHOUSE_DATABASE cannot be empty",
            ));
        }
        if username.trim().is_empty() {
            return Err(SiemError::configuration(
                "CLICKHOUSE_USERNAME cannot be empty",
            ));
        }
...
impl DatabaseManager {
    ...
    pub async fn get_connection(&self) -> SiemResult<DatabaseConnection> {
        let connection = self
            .retry_policy
            .execute(|| async { self.get_connection_internal().await })
            .await?;
    ...
    where
        F: FnOnce(
            ...
            -> std::pin::Pin<Box<dyn std::future::Future<Output = SiemResult<T>> + Send>>,
    {
    ...
    pub async fn table_exists(&self, table_name: &str) -> SiemResult<bool> {
        ...
        let result = client
            ...
            .await
            .map_err(|e| {
                SiemError::database_with_source(
