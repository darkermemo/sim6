...
export function RulesPage() {
  ...
  const activeTab = new URLSearchParams(location.search).get('tab') || 'definition';
  ...
  // Fetch rules list
  const { data: rulesData, isLoading: loadingList } = useQuery({
    queryKey: ['rules', tenantId, urlState.q, cursor],
  ...
  React.useEffect(() => {
    if (rulesData) {
      if (cursor) {
        setAllRules(prev => [...prev, ...rulesData.data]);
      } else {
  ...
  const { data: ruleDetails, isLoading: loadingDetails } = useQuery({
  ...
  const getWatermarkWindow = () => {
    ...
    // For demo, use last 24h as watermark
    const watermark = new Date(now.getTime() - 24 * 60 * 60 * 1000);
