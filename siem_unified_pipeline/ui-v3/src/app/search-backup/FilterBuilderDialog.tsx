...
export function FilterBuilderDialog({ open, onOpenChange, onApply, tenantId }: FilterBuilderDialogProps) {
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 z-50 bg-black data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0" />
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-full max-w-6xl max-h-[95vh] -translate-x-1/2 -translate-y-1/2 ring-2 ring-border bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 shadow-2xl duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 rounded-lg flex flex-col isolate">
          ...
          <div className="flex flex-1 min-h-0">
            ...
            <div className="flex-1 min-w-0 overflow-y-auto bg-white dark:bg-slate-900">
                ...
                <TabsContent value="saved" className="space-y-4">
                  <div className="space-y-4">
                    {SAVED_SEARCHES.map(search => (
                      <Card key={`saved-${search.id}`} className="cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700" onClick={() => loadSavedSearch(search)}>
              ...
              <div className="space-y-2">
                              ...
                              <code className="text-xs bg-slate-100 dark:bg-slate-800 p-1 rounded">{search.query}</code>
                            </div>
                            <ActionButton 
                              variant="ghost" 
                              size="icon" 
            ...
            <div className="w-80 border-l border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 flex flex-col">
              <div className="sticky top-0 p-6 bg-slate-100 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
