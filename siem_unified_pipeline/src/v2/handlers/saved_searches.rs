...
pub async fn list_saved(State(st): State<Arc<AppState>>, Query(q): Query<ListQ>) -> Result<Json<ListResp>, crate::error::PipelineError> {
    ...
    let db = st.events_table.split_once('.').map(|(d,_)| d).unwrap_or("dev");
    let table = format!("{}.saved_searches", db);
    let mut sql = format!("SELECT tenant_id,saved_id,name,updated_at FROM {} WHERE tenant_id=? ", table);
    ...
    let rows: Vec<(String,String,String,u64)> = match qq.fetch_all().await {
        Ok(v) => v,
        Err(e) => {
            let es = e.to_string();
            // Gracefully handle table-not-found/missing table errors
            if es.contains("Code: 60") || es.to_ascii_lowercase().contains("doesn't exist") || es.contains("UNKNOWN_TABLE") {
                return Ok(Json(ListResp{ items: vec![], next_cursor: None }));
            }
            return Err(crate::error::PipelineError::database(format!("list saved: {e}")));
        }
    };
...
pub async fn get_saved(State(st): State<Arc<AppState>>, Path(id): Path<String>, Query(q): Query<std::collections::HashMap<String,String>>) -> Result<Json<serde_json::Value>, crate::error::PipelineError> {
    ...
    let db = st.events_table.split_once('.').map(|(d,_)| d).unwrap_or("dev");
    let table = format!("{}.saved_searches", db);
    let sql = format!("SELECT tenant_id,saved_id,name,dsl,updated_at FROM {} WHERE tenant_id=? AND saved_id=? LIMIT 1", table);
    let row: Option<(String,String,String,String,u64)> = st.ch.query(&sql).bind(&tenant).bind(&id).fetch_optional().await.map_err(|e| crate::error::PipelineError::database(format!("get saved: {e}")))?;
...
pub async fn create_saved(State(st): State<Arc<AppState>>, Json(b): Json<CreateReq>) -> Result<Json<OkId>, crate::error::PipelineError> {
    ...
    let db = st.events_table.split_once('.').map(|(d,_)| d).unwrap_or("dev");
    let table = format!("{}.saved_searches", db);
    let sql = format!("INSERT INTO {} (tenant_id,saved_id,name,dsl,created_at,updated_at) VALUES (?,?,?,?,?,?)", table);
    st.ch.query(&sql).bind(&b.tenant_id).bind(&sid).bind(&b.name).bind(&dsl_s).bind(now).bind(now).execute().await.map_err(|e| crate::error::PipelineError::database(format!("saved insert: {e}")))?;
...
pub async fn update_saved(State(st): State<Arc<AppState>>, Path(id): Path<String>, Json(b): Json<UpdateReq>) -> Result<Json<OkId>, crate::error::PipelineError> {
    ...
    let db = st.events_table.split_once('.').map(|(d,_)| d).unwrap_or("dev");
    let table = format!("{}.saved_searches", db);
    let sql = format!("ALTER TABLE {} UPDATE {} WHERE tenant_id='{}' AND saved_id='{}'", table, sets.join(","), b.tenant_id.replace("'","''"), id.replace("'","''"));
...
pub async fn delete_saved(State(st): State<Arc<AppState>>, Path(id): Path<String>, Query(q): Query<std::collections::HashMap<String,String>>) -> Result<Json<OkId>, crate::error::PipelineError> {
    ...
    let db = st.events_table.split_once('.').map(|(d,_)| d).unwrap_or("dev");
    let table = format!("{}.saved_searches", db);
    let sql = format!("ALTER TABLE {} DELETE WHERE tenant_id=? AND saved_id=?", table);
    st.ch.query(&sql).bind(&tenant).bind(&id).execute().await.map_err(|e| crate::error::PipelineError::database(format!("saved delete: {e}")))?;
