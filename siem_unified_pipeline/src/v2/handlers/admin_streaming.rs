...
pub async fn streaming_status(State(st): State<Arc<AppState>>, Query(q): Query<StatusQ>) -> Result<Json<StreamingStatus>, crate::error::PipelineError> {
...
pub async fn streaming_reclaim(State(st): State<Arc<AppState>>, Query(q): Query<StatusQ>) -> Result<Json<serde_json::Value>, crate::error::PipelineError> {
    let cm = st.redis.clone().ok_or_else(|| crate::error::PipelineError::service_unavailable("redis not configured"))?;
    let mut conn = cm.clone();
    let key = format!("siem:events:{}", q.tenant_id);
    let group = std::env::var("STREAM_GROUP").unwrap_or_else(|_| "gr1".to_string());
