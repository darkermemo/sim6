...
export function RuleEditor({
  ...
  const [draftSaveTimer, setDraftSaveTimer] = React.useState<number | null>(null);
  const [lastCompiledHash, setLastCompiledHash] = React.useState<string>('');
  const [showCompileButton, setShowCompileButton] = React.useState(false);
  ...
  React.useEffect(() => {
    if (rule) {
      ...
      // Check for draft
      const draftKey = `rule-draft-${rule.rule_id}`;
      const draft = localStorage.getItem(draftKey);
      if (draft) {
        try {
          const draftData = JSON.parse(draft);
          if (window.confirm('Restore unsaved changes from your previous session?')) {
            setName(draftData.name || rule.name);
            setDescription(draftData.description || '');
            setSeverity(draftData.severity || rule.severity);
            setDsl(draftData.dsl || '');
            setSigmaYaml(draftData.sigma_yaml || '');
            setWatermarkSec(draftData.watermark_sec || 120);
            setThrottleSeconds(draftData.throttle_seconds || 0);
            setAlertKey(draftData.alert_key || 'coalesce(user, src_ip, host)');
  ...
  React.useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      localStorage.setItem(`rule-draft-${rule.rule_id}`, JSON.stringify(draft));
  ...
  }, [name, description, severity, kind, dsl, sigmaYaml, watermarkSec, throttleSeconds, alertKey, tags, rule, isDirty, draftSaveTimer]);
  ...
  const handleContentChange = (content: string, isNative: boolean) => {
    ...
    setIsDirty(true);
    setShowCompileButton(true);
  };

  // Explicit compile
  const handleCompile = () => {
    const content = kind === 'NATIVE' ? dsl : sigmaYaml;
    if (content.trim()) {
      onCompile(kind, content);
      setLastCompiledHash(generateHash(content));
      setShowCompileButton(false);
    }
  };
  ...
  const handleSave = () => {
    ...
    // Check if content has been compiled
    const currentContent = kind === 'NATIVE' ? dsl : sigmaYaml;
    const currentHash = generateHash(currentContent);
    if (currentHash !== lastCompiledHash) {
      alert('Please compile your changes before saving.');
      return;
    }
    ...
    if (rule) {
      localStorage.removeItem(`rule-draft-${rule.rule_id}`);
    }
  };
  ...
  const currentContent = kind === 'NATIVE' ? dsl : sigmaYaml;
  const currentHash = generateHash(currentContent);
  const isCompiled = currentHash === lastCompiledHash;
