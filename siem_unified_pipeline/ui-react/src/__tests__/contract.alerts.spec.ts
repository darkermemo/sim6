...
describe('Alerts API Contract', () => {
  describe('AlertsListRes', () => {
    it('matches wire format for list response', () => {
      const response: AlertsListRes = wireAlerts as any;
      ...
      // Check alerts array (raw format has .alerts property)
      const alertsData = (wireAlerts as any).alerts;
      expect(alertsData).toBeInstanceOf(Array);
      expect(alertsData.length).toBe(3);
      
      // Check first alert structure
      const alert: Alert = alertsData[0];
    ...
    it('handles all severity levels', () => {
      const severities = (wireAlerts as any).alerts.map((a: any) => a.severity);
    ...
    it('handles all status values', () => {
      const statuses = (wireAlerts as any).alerts.map((a: any) => a.status);
