...
# Consolidated Full System Regression Test Plan
...
echo "=============================================================="
echo "CONSOLIDATED FULL SYSTEM REGRESSION TEST PLAN"
echo "=============================================================="
...
generate_jwt_token() {
    ...
    local tenant_id=$2
    local roles=$3
    local secret="this-is-a-very-long-secure-random-string-for-jwt-signing-do-not-use-in-production"
    
    # Create JWT payload
    local exp=$(date -d "+1 hour" +%s)
...
EOF
    ...
    # For this test, we'll use a pre-generated token approach
    # In production, use proper JWT generation
    echo "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIkdXNlcl9pZCIsInRpZCI6IiR0ZW5hbnRfaWQiLCJyb2xlcyI6WyJTdXBlckFkbWluIl0sImV4cCI6MTczNTc1NTM1Mn0.dummy-token-for-testing"
}
...
prepare_test_environment() {
    ...
    # Check Ingestor
    if ! wait_for_service "$SIEM_INGESTOR_URL/health" "SIEM Ingestor"; then
        log "WARN" "SIEM Ingestor may not be available - some tests may fail"
    fi
    ...
    SUPERADMIN_TOKEN=$(generate_jwt_token "superadmin" "system" '["SuperAdmin"]')
    TENANT_A_ADMIN_TOKEN=$(generate_jwt_token "admin-a" "tenant-A" '["Admin"]')
    TENANT_B_ANALYST_TOKEN=$(generate_jwt_token "analyst-b" "tenant-B" '["Analyst"]')
    
    # Save tokens to files for other services to use
    echo "$SUPERADMIN_TOKEN" > superadmin_token.txt
    echo "$TENANT_A_ADMIN_TOKEN" > admin_a_token.txt
    echo "$TENANT_B_ANALYST_TOKEN" > analyst_b_token.txt
...
test_suite_a() {
    ...
    # This is a simplified test - in practice, you'd need to send many requests quickly
...
test_suite_b() {
    ...
    local test_event='{"events": [{"event_id": "test-b1", "tenant_id": "tenant-A", "event_timestamp": '$(date +%s)', "raw_event": "Jan 01 12:00:00 testhost sshd[1234]: Failed password for user from 192.168.1.100", "source_ip": "192.168.1.100"}]}'
    ...
    if [[ "$response" == "201" ]]; then
        ...
        count=$(echo "$clickhouse_response" | jq -r '.data[0]["count()"]')
        
        if [[ "$count" == "1" ]]; then
            record_test_result "B-1" "Syslog Ingestion" "PASS"
        else
            record_test_result "B-1" "Syslog Ingestion" "FAIL" "Event not found in ClickHouse"
        fi
    else
        record_test_result "B-1" "Syslog Ingestion" "FAIL" "API returned $response"
    fi
    
    # Test B-2: HTTP Ingestion (same as B-1 in our simplified setup)
    log "INFO" "Running test B-2: HTTP Ingestion"
    local test_event2='{"events": [{"event_id": "test-b2", "tenant_id": "tenant-A", "event_timestamp": '$(date +%s)', "raw_event": "HTTP POST /api/login failed for user admin", "source_ip": "192.168.1.200"}]}'
    ...
    if [[ "$response" == "201" ]]; then
        ...
        if [[ "$count" == "1" ]]; then
            ...
            record_test_result "B-2" "HTTP Ingestion" "FAIL" "Event not found in ClickHouse"
        fi
    else
...
test_suite_c() {
    ...
    # Test C-1: Intelligent Parsing (simplified - log source configuration)
...
test_suite_e() {
    ...
    local alert_data='{"alerts": [{"alert_id": "test-alert-for-case", "tenant_id": "tenant-A", "rule_id": "test-rule-123", "rule_name": "Test Rule for Case", "event_id": "test-event-123", "alert_timestamp": '$(date +%s)', "severity": "Medium", "description": "Test alert for case management", "raw_event": "Test event data"}]}'
    ...
    if [[ "$response" == "201" ]]; then
        CREATED_CASE_ID=$(cat /tmp/case_response.json | jq -r '.case_id')
    ...
    if [[ -n "$CREATED_CASE_ID" ]]; then
        ...
        if [[ "$response" == "200" ]]; then
            evidence_count=$(cat /tmp/case_get_response.json | jq -r '.evidence | length')
...
test_suite_f() {
    ...
    if [[ "$response" == "200" ]]; then
        config_content=$(cat /tmp/agent_config.json | jq -r '.policy_config')
...
test_suite_j() {
    ...
    if [[ "$response" == "201" ]]; then
        STATEFUL_RULE_ID=$(cat /tmp/stateful_rule.json | jq -r '.rule_id')
    ...
    if [[ -n "$STATEFUL_RULE_ID" ]]; then
        ...
        for i in {1..4}; do
            local event_data='{"events": [{"event_id": "stateful-test-'$i'", "tenant_id": "tenant-A", "event_timestamp": '$(date +%s)', "raw_event": "failed login attempt from user admin", "source_ip": "192.168.1.50"}]}'
        ...
        alert_count=$(echo "$alerts_response" | jq -r '.data | length')
        
        if [[ "$alert_count" -gt 0 ]]; then
            record_test_result "J-2" "Stateful Detection" "PASS"
        else
            record_test_result "J-2" "Stateful Detection" "FAIL" "No alerts generated from stateful rule"
        fi
    else
...
test_suite_k() {
    ...
    if [[ "$response" == "201" ]]; then
        sql_query=$(cat /tmp/sigma_rule.json | jq -r '.rule.query')
        if [[ "$sql_query" != "null" && "$sql_query" != "" ]]; then
            SIGMA_RULE_ID=$(cat /tmp/sigma_rule.json | jq -r '.rule.rule_id')
    ...
    if [[ -n "$SIGMA_RULE_ID" ]]; then
        # Send matching event
        local matching_event='{"events": [{"event_id": "sigma-test-event", "tenant_id": "tenant-A", "event_timestamp": '$(date +%s)', "raw_event": "User login failed for admin user", "source_ip": "192.168.1.60"}]}'
        ...
        sigma_alerts=$(echo "$alerts_response" | jq -r --arg rule_id "$SIGMA_RULE_ID" '.data[] | select(.rule_id == $rule_id)')
